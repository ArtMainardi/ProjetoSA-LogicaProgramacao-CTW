...
public class EquipamentoService {
    ...
    public Equipamento atualizar(Equipamento modifiedEquipamento, int id){
        ...
        if(modifiedEquipamento.getNome().trim().isEmpty() || modifiedEquipamento.getCategoria().trim().isEmpty() || newEquipamento.getFabricante().trim().isEmpty() || 
                modifiedEquipamento.getModelo().trim().isEmpty() || newEquipamento.getSetorInstalado().trim().isEmpty() || 
                modifiedEquipamento.getDataInstalacao().trim().isEmpty() || newEquipamento.getStatus().trim().isEmpty()){
            throw new RuntimeException("ERRO: não é permitido cadastrar equipamentos com algum dado vazio!");
        }
        if(!status.equals("Operando") && !status.equals("Operando") && !status.equals("Operando")){
            throw new RuntimeException("ERRO: valor de 'status' inválido!");
